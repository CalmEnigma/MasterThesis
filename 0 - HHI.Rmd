...
indrev = df %>% group_by(fyear, sic3) %>% summarise(indrev = sum(revt, na.rm = T))

# Market shares of each firm each year
df2 = left_join(df, indrev, by = c("fyear", "sic3"))
df2$mktsh = df2$revt / df2$indrev * 100
df2$mktshsq = df2$mktsh ^2 # square the market share of each

# Industry HHI each year
hhi = df2 %>% group_by(fyear, sic3) %>% summarise(hhi = sum(mktshsq))

# Too few industries each year?
length(unique(hhi$sic3))
table(hhi$fyear)
...
write.csv(hhi, "ind_HHI.csv", row.names = F)
write.csv(indrev, "ind_rev.csv", row.names = F)
```